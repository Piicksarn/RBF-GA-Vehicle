...
function newPosition() {
  ...
  ang = result;
  var x = math.add(math.cos(ang_phi + (result * math.pi / 180)), math.sin(result * math.pi / 180) * math.sin(ang_phi));
  x = math.add(vehicle.xPosition, x);
  var y = math.subtract(math.sin(ang_phi + (result * math.pi / 180)), math.sin(result * math.pi / 180) * math.cos(ang_phi));
  y =  math.add(vehicle.yPosition, y);
  ang_phi = ang_phi - math.asin(2 * math.sin(result * math.pi / 180) / 3);
  return [x, y];
}
...
function sliceData(dataArray) {
  ...
  return [dataArray[0], dataArray.slice(1,neuroNum + 1), mean, sigma];
}
